...
> **Enhancement notes:** this file already had strong coverage (all six anomalies with sequence diagrams, the full ANSI matrix, 2PL/MVCC/SSI internals, and an interview routing diagram), so the additions below fill specific gaps rather than restructure anything:
> - Added **§7 🆕 "Choosing an isolation level for a real workload"** — the practical "which level for which workload, and what does the wrong choice cost" guidance that was previously only implied by scattered asides. Includes an illustrative (explicitly labeled, non-benchmarked) example of Serializable's retry cost under contention, plus an "if X then Y" recall list.
> - Added a **🆕 "2PL vs. MVCC at a glance"** comparison table + diagram in §5, contrasting the two implementation families side by side (they existed as separate diagrams before, but weren't directly compared).
> - Added a **🆕 "Anomaly-elimination ratchet" diagram** in §3, showing what guarantee each ANSI level *adds* over the previous one, as a visual companion to the existing matrix.
> - Added two mnemonics for interview recall: "Ugly Code Rarely Scales" for the ANSI level order (§3), and a one-line 2PL-vs-MVCC recall sentence (§5).
> - Left untouched: all six anomaly write-ups and their sequence diagrams (§2), the ANSI matrix and database-quirks table (§3–4), the 2PL/MVCC/SSI explanations themselves (§5), the interview-routing diagram (§6), and the original cheat sheet content (only appended two bullets) — these were already clear, accurate, and example-rich.
...
**Mnemonic for the four ANSI levels in order — "Ugly Code Rarely Scales":** **U**ncommitted → **C**ommitted → **R**epeatable → **S**erializable. Each step trades a little concurrency for one more guarantee. If you can only remember one fact per level, remember what it *adds* over the previous one: Read Committed adds "no dirty reads," Repeatable Read adds "no stale re-reads," Serializable adds "no cross-row invariant violations" (closing the write-skew gap that even Snapshot Isolation leaves open).

#### 🆕 Anomaly-elimination ratchet

Each level only ever *adds* a guarantee — it never gives one back. This diagram makes that ratchet explicit, so you can derive the whole matrix from §3 instead of memorizing it cell by cell:

```mermaid
graph LR
    RU["Read Uncommitted<br/>allows all 5 read anomalies"]
    RC["Read Committed<br/>+ no dirty reads"]
    RR["Repeatable Read /<br/>Snapshot Isolation<br/>+ no stale re-reads, no phantoms*"]
    SER["Serializable<br/>+ no write skew (the last gap)"]
    RU -->|blocks dirty reads| RC
    RC -->|snapshot per txn| RR
    RR -->|full conflict detection| SER
```

`*` phantom-read protection at Repeatable Read is engine-specific (see §4) — the ANSI standard itself only promises it at Serializable.
...
#### 🆕 2PL vs MVCC at a glance

Both mechanisms can implement the same isolation level (e.g., Repeatable Read) — they just make opposite bets about who pays the cost.

| Dimension | 2PL (pessimistic) | MVCC (optimistic-ish) |
|---|---|---|
| Core idea | Lock the row before touching it | Keep old versions; give each reader a snapshot |
| Readers vs. writers | Block each other | Never block each other |
| Writers vs. writers | Block (wait for lock) | Block, or abort-and-retry (first-committer-wins) |
| Storage cost | None extra — locks are in-memory | Extra: old row versions pile up until vacuumed |
| Failure mode under load | Waiting, then deadlocks | Bloat (Postgres: `VACUUM` backlog), serialization-failure retries |
| Who uses it as the default | SQL Server (locking mode), classic textbook engines | PostgreSQL, MySQL InnoDB, Oracle, CockroachDB |

```mermaid
graph TD
    subgraph "2PL: writer blocks reader"
        L1[Reader wants row X] -->|must wait| L2[Writer holds exclusive lock on X]
        L2 -->|lock released at commit| L1
    end
    subgraph "MVCC: writer never blocks reader"
        M1[Reader opens snapshot @ t] --> M2[Reads version committed before t]
        M3[Writer creates new version @ t+1] -.->|invisible to the t-snapshot| M1
    end
```

**One-line recall: 2PL makes the second transaction *wait*; MVCC makes the second transaction *not see it yet* (and settles write/write conflicts at commit time instead of up front).**
...
## 7. 🆕 Choosing an isolation level for a real workload — and its cost

The interview trap is treating this as "pick the strongest level to be safe." In practice you pick the *weakest* level that protects the specific invariant your transaction depends on, because every step up the ladder in §1 removes concurrency. A "safe by default" answer that ignores cost reads as junior; naming the trade-off reads as senior.

| Workload | Pick | Why |
|---|---|---|
| High-throughput CRUD (add-to-cart, view profile, log an event) | **Read Committed** | No cross-row invariant to protect; dirty reads are the only bug class that matters, and Read Committed already kills those. This is every mainstream database's default for a reason. |
| Single counter / single-row increment (view count, rate limiter) | **Read Committed + atomic `UPDATE ... SET x = x + 1` or CAS** | Fixes lost update without paying for a stronger, whole-transaction isolation level — see §2.5. |
| Report or backup job reading many rows over several seconds | **Repeatable Read / Snapshot Isolation** | Needs one consistent point-in-time view (no read skew), but never writes, so write skew can't happen — Serializable would add cost here for no benefit. |
| A transaction that enforces an invariant spanning two or more rows (on-call staffing, room booking, combined account balance) | **Serializable, scoped to just that transaction** — or better, a schema-level constraint (`UNIQUE`, `CHECK`, exclusion constraint) that turns the invariant into a single-row check | This is the only case in the table where write skew is actually reachable. Don't pay the Serializable tax on the other 95% of your transactions to protect the 5% that need it. |

**Illustrative cost example** (numbers are for intuition, not a benchmark — actual retry rates depend entirely on contention on the specific rows involved): imagine a checkout service that wraps its *entire* order-processing flow in Serializable instead of scoping it to just the inventory-decrement step. Under peak load, many of those transactions touch overlapping rows (popular SKUs, shared counters), so the database's conflict detector (SSI, §5.3) starts aborting a real, non-trivial share of them with a serialization-failure error — each one forced to retry from the top, adding latency and burning CPU on work that's thrown away. Scoping Serializable to only the few statements that read-then-write the invariant (e.g., the stock check-and-decrement) keeps that retry cost confined to the code path that actually needs it, instead of taxing checkout, browsing, and everything else in the same transaction boundary.

**"If X, then Y" recall list:**
- If the transaction touches exactly one row → Read Committed is enough.
- If it re-reads data within itself and must not see it change mid-transaction → Repeatable Read / Snapshot Isolation.
- If it reads several rows to enforce an invariant, then writes to different rows → that's write skew territory → Serializable (scoped) or a schema constraint.
- If it's read-only and long-running (report/backup) → Snapshot Isolation, not a lock — locks would needlessly block the writers it's not even conflicting with.
- If you're not sure which anomaly applies → walk the routing diagram above before reaching for a fix.
...
- Mnemonic for the ladder: **"Ugly Code Rarely Scales"** = Uncommitted → Committed → Repeatable → Serializable. Each level only ever *adds* a guarantee, never removes one (§3's 🆕 ratchet diagram).
- Picking a level is a cost decision, not a safety maximization: default to Read Committed, reach for Repeatable Read/Snapshot Isolation for read-only multi-row consistency, and scope Serializable to the *specific* transaction protecting a cross-row invariant — never the whole database (§7).
