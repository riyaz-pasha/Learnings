...
> **Enhancement notes:** this pass added — (1) a PACELC tie-in and a spectrum mnemonic in §1; (2) two new 🆕 sequence diagrams in §4 showing a causal-consistency violation and the vector-clock-based fix (comment before its dependency); (3) a new 🆕 "fix" sequence diagram in §5 showing sticky routing/version-token resolving the read-your-writes violation that was already diagrammed; (4) illustrative numeric examples (cross-region consensus latency in §2, a 200ms replication-lag window in §6, K/T bounded-staleness numbers in §7) — all labeled illustrative since exact figures are environment-dependent; (5) a new 🆕 §9 comparison table (model × guarantee × example use case × relative cost) plus two extra "if X then Y" interview bullets (bank balance → linearizable, like count → eventual); (6) a light clarity pass tightening the Bounded Staleness paragraph. Everything else — the existing structure, tables, TAO case study, tunable-consistency section, and decision diagrams — was left as-is because it already worked.
...
**🆕 How this maps to PACELC**: CAP only tells you what happens *during* a partition. PACELC adds the "else" — even with **no** partition, you still trade latency against consistency on every normal-case request. Every model on this spectrum is a different point on that same dial: linearizable pays a latency tax on *every* op (coordination even with no partition in sight), eventual pays nothing and stays fast, and everything in between (sequential, causal, session) is a deliberate partial trade. When an interviewer asks "where does this fit on PACELC," the answer is: consistency models are the menu; PACELC is the axis you're choosing a point on.

**🆕 Mnemonic** for the order strongest → weakest — **"Lisa Sees Causes, Sometimes Eventually"** → **L**inearizable, **S**equential, **C**ausal, **S**ession, **E**ventual.
...
**Cost**: every read and write must coordinate with enough of the system to rule out a stale answer — this is exactly what makes it expensive across a WAN. Achieving linearizability across geographically distributed replicas requires consensus (Paxos/Raft) or synchronized clocks (Spanner's TrueTime) — see [9.9](9.9%20Distributed%20Transactions%20and%20Consensus.md). **Illustrative example**: a consensus round trip between replicas on different continents commonly adds on the order of 50-150ms to every write (exact number depends on distance and network path) — that's latency paid on *every single operation*, not just during a failure, which is the real reason teams avoid linearizing anything that doesn't strictly need it.
...
#### 🆕 Causal-consistency violation — and how causal consistency prevents it

Without causal tracking, a fast replica can deliver a reply before the comment it depends on has replicated there yet:
...
sequenceDiagram
    ...
    Author->>System: write(Comment A: "Tacos!")
    Author->>System: write(Comment B: "Tacos sound great!", replies to A)
    System->>Viewer: replicates B first — routed via a faster path than A
    Viewer-->>Viewer: renders "Tacos sound great!" with no parent comment visible
    Note over Viewer: VIOLATION — B is shown before the A it causally depends on
```

Causal consistency fixes this by attaching each write's dependencies (via a vector clock) and having the receiving replica **hold** a write until its dependencies have already been applied locally:
...
sequenceDiagram
    ...
    Author->>System: write(Comment A) — tagged {A:1}
    Author->>System: write(Comment B, depends on {A:1}) — tagged {A:1,B:1}
    System->>Viewer: replicates B, attaches dependency {A:1}
    Viewer->>Viewer: checks — has {A:1} already been applied here?
    alt A not yet applied locally
        Viewer->>Viewer: B is buffered — not shown yet
    else A already applied
        Viewer->>Viewer: B is delivered immediately
    end
    Note over Viewer: FIXED — B only ever becomes visible after A is visible
```
...
#### 🆕 The fix — sticky routing / version token
...
sequenceDiagram
    ...
    Client->>Primary: write(profile_pic = new.jpg)
    Primary-->>Client: ack + version token v42
    Client->>Primary: read(profile_pic) — sticky-routed back to Primary,<br/>or to any replica that confirms it has applied v42
    Primary-->>Client: new.jpg
    Note over Client: FIXED — client either reads from the primary directly,<br/>or carries v42 and refuses an answer from a replica that hasn't reached it
```
...
**🆕 Concrete example**: say a write acks in 5ms at the primary, but async replication to a follower typically takes 200ms to catch up (illustrative — actual lag depends on network and load). Any read that lands on that follower within that 200ms window can return the old value — a real, if brief, window where "the write already happened" and "the read shows the old value" are both true at once. That 200ms window is the entire price (and the entire benefit) of eventual consistency: the write path never waits for it, but any read can fall into it.
...
**Bounded Staleness, defined** (the one term in that table that isn't self-explanatory): it's a middle ground between strong and eventual. You set a hard limit — for example, "never more than **K = 100 versions** behind" or "never more than **T = 5 seconds** stale" (illustrative numbers; Cosmos DB lets you configure both, whichever is hit first). Every replica is guaranteed to stay inside that limit, and reads within the window are still monotonic and prefix-consistent — so instead of open-ended staleness, you get a known, bounded amount of it. Cosmos DB turned this into a first-class, explicitly tunable knob rather than an unmanaged side effect of replication lag — name it if an interviewer asks "is there anything between strong and eventual that still gives a hard, numeric guarantee?"
...
## 9. 🆕 Comparison table — model × guarantee × example × cost

| Model | Guarantee | Example use case | Relative cost |
|---|---|---|---|
| **Linearizable** | Real-time-respecting single-copy illusion | Distributed lock, username uniqueness, bank balance | Highest — consensus/TrueTime round trip on every op (illustrative: 50-150ms cross-region) |
| **Sequential** | One global order for everyone, but not tied to wall-clock time | Replicated state machines that tolerate reordering vs. real time | High — still needs one agreed order, just not a real-time bound |
| **Causal** | Causally-related ops stay ordered; concurrent ops can reorder freely | Comment/reply threads, collaborative editors | Medium — vector-clock bookkeeping, no global coordinator needed |
| **Session** (read-your-writes / monotonic reads / monotonic writes / writes-follow-reads) | Per-client guarantees only, not global | "I posted this, why don't I see it"; a user's own feed/profile | Low — sticky routing or a version token, no cluster-wide coordination |
| **Eventual** | Converges *eventually*, no bound in the meantime | Like counts, follower counts, view counts | Lowest — no coordination, fastest writes, accepts a staleness window |

Cost is relative/illustrative, meant for ordering the models against each other, not as a benchmark you can quote verbatim.
...
- 🆕 "Bank balance" or "inventory count at checkout" → Linearizable (or Serializable transactions, pair with 9.2) — money and inventory can't double-spend, so this is one of the few reads/writes worth paying the coordination tax for.
- 🆕 "Like count" or "view count" → Eventual — a brief, invisible staleness window (see §6's 200ms example) is a fine trade for not blocking every like/view on a cluster-wide ack.
...
- Spectrum, strongest to weakest: **Linearizable → Sequential → Causal → Session (read-your-writes / monotonic reads / monotonic writes / writes-follow-reads) → Eventual.** Mnemonic: 🆕 *"Lisa Sees Causes, Sometimes Eventually."*
