...
> **Enhancement notes:** this file already covered the core structures, diagrams, and trade-offs in depth, so the pass below is additive, not a rewrite. Marked with 🆕:
> - Spelled out that **LSM = Log-Structured Merge**-Tree, with a one-line mnemonic tying the acronym directly to the write path (§3, and a combined B-Tree/LSM mnemonic callout before §4).
> - Added a concrete write-amplification number for B-Trees (a ~100-byte row update forcing a 40x–160x page rewrite, §2) and a concrete Bloom-filter number (a ~1% false-positive rate turning ~100 wasted disk reads into ~1, §3).
> - Added illustrative numbers to the compaction-strategies discussion (Cassandra's size-tiered merge threshold, illustrative space-amplification and write-amplification ranges for size-tiered vs. leveled, §3) and a new comparison table for the write/read/space amplification triangle (§3), all explicitly labeled illustrative where the exact figures are configuration-dependent.
> - Left the existing page/memtable/SSTable deep dives, skip-list walkthrough, mermaid diagrams (B-Tree structure, LSM write path, bloom-filter read path, tombstone bug, routing diagram), head-to-head table, and "who uses which" table untouched — they were already at the right depth and voice.
...
To make that concrete: changing a single ~100-byte row still forces the whole containing page — 4KB, 8KB, or 16KB depending on the engine — to be rewritten, since a page is the smallest unit of I/O (§1.1). That alone is **40x–160x** write amplification for that one flush, before a single ancestor-page split ever happens. This is the cost an LSM-Tree's append-only write path is specifically designed to avoid.
...
Spelled out, LSM stands for **Log-Structured Merge**-Tree — and that name is literally the write path in three words: **Log** it (WAL), keep it **Structured**/sorted (memtable), **Merge** it later (compaction). If you only remember the acronym expansion under pressure, you can reconstruct the whole write path from it.
...
Put a number on it: RocksDB commonly targets roughly a **1% false-positive rate** per Bloom filter (illustrative — it's tunable, and cost scales with bits-per-key). Looking up a key that doesn't exist in a given SSTable, that filter correctly says "not here" about 99 times out of 100 — turning what would've been 100 wasted disk reads into 1. The 1 remaining case is the false positive: a disk read that comes back empty. That's the entire economic case for shipping a Bloom filter with every SSTable — a few bits per key in RAM buys back almost all of the read amplification from checking files that never had the key.
...
#### 🆕 Compaction strategies, with illustrative numbers

- **Size-tiered**: Cassandra's default trigger is a small pile of similarly-sized SSTables — commonly around 4 to 32 of them (`min_threshold`/`max_threshold`, illustrative defaults, tunable) — before they're merged into one. Because multiple overlapping tiers of the same key can coexist until that merge fires, space amplification can transiently balloon to roughly **2x or more** of the logical data size right before a big compaction runs (illustrative, depends on write pattern) — which is why Cassandra operators are told to keep meaningful free disk headroom, not run near-full.
- **Leveled**: each level is sized roughly 10x the level above it (already noted in the table above), and within a level, key ranges don't overlap — so at steady state a key lives in at most one SSTable per level, keeping space amplification low, illustratively in the ~1.1x–1.3x range. The cost is on the write side: a piece of data gets rewritten every time it's merged down into the next level, and across a key's full lifetime that's often cited in the **10x–30x** write-amplification range (illustrative, workload- and configuration-dependent) — noticeably worse than size-tiered's write cost, in exchange for far fewer SSTables to check per read.
...
#### 🆕 The triangle as one table (illustrative magnitudes, not benchmarks)

| Engine | Write amplification | Read amplification | Space amplification |
|---|---|---|---|
| B-Tree | Low — ~1 page write per logical write, worse only when a split cascades | Low — one root-to-leaf traversal | Low — one copy of the data, in place |
| LSM, size-tiered | Lowest of the three | Highest — may check memtable + several SSTables per level tier | Highest — illustratively ~2x+ right before a big merge fires |
| LSM, leveled | Highest — illustratively 10x–30x over a key's lifetime as it's rewritten level by level | Lower than size-tiered — at most ~1 SSTable per level to check at steady state | Lowest of the LSM options — illustratively ~1.1x–1.3x |

Read this table as: pick any column, and the row that wins it is the row that loses hardest on at least one of the other two — that's the trade-off, not a bug in any one design.
...
> **🆕 Mnemonic — say this out loud once and it sticks:**
> **B-Tree** = **B**alanced tree, update the **B**yte where it already lives (in place).
> **LSM-Tree** = **L**og it (WAL), keep it **S**orted (memtable), **M**erge it later (compaction) — the acronym *is* the write path.
